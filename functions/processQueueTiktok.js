  ...
  try {
    ...
    for (const upload of (pendingUploads || [])) {
      try {
        ...
        // Already published today on TikTok? (use published_content as source of truth)
        ...
        if (remaining <= 0) {
          console.log(`[TikTok] Skip ${upload.clip_id}: user ${userId} out of slots (tier=${tier})`);
          continue;
        }
